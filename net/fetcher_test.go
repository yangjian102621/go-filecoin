...
func TestFetchHappyPath(t *testing.T) {
	...
	rtBlocks, err := fetcher.GetBlocks(context.Background(), originalCids.ToSlice())
	require.NoError(err)
	require.Equal(3, len(rtBlocks))
	rtCids := types.NewSortedCidSet(rtBlocks[0].Cid(), rtBlocks[1].Cid(), rtBlocks[2].Cid())

	require.True(originalCids.Equals(rtCids))
}
